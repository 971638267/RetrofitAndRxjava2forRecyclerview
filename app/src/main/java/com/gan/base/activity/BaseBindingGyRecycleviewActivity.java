...
import com.gan.base.adapter.BaseBindingAdapter;
...
public abstract class BaseBindingGyRecycleviewActivity<T, B extends ViewDataBinding> extends BaseActivity implements GyRecycleView.RefreshLoadMoreListener {
    @Override
    protected void afterView() {
        recycleView = (GyRecycleView) findViewById(R.id.pager_base_recycleview);
    }
    ...
    private BaseBindingAdapter<T, B> mAdapter;
    private RecycleviewSubscriberOnNextListener<List<T>> getTopMovieOnNext;
    public ObservableArrayList<T> dataAllList = new ObservableArrayList<T>();
  ...
  /*  public BaseRecyclerViewAdapter<T> getRecyclerViewAdapter() {
    ...
    public BaseBindingAdapter<T, B> getRecyclerViewAdapter() {
        return new BaseBindingAdapter<T, B>(this, dataAllList) {
            @Override
    ...
    protected abstract void doItemUI(B binding, T t);
